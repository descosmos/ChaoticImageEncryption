function encoded_img = encode_img(original_img, complementary_principle)
%{
    ===============================================
                    encode_image
    ===============================================
    ...
    3. 
%}
    encoded_img = original_img;

    function encoded_one_img = encode_one_img(image, complementary_principle)
        [M,N] = size(image);
        encoded_dna_quence = uint8(zeros(M, N*4));
        encoded_one_img = uint8(zeros(M,N));
        ...
        for i=1:M
            for j=1:N
                ...
                encoded_one_img(i,j) = pixel;
            end
        end
    end
    
    red_encoded_img = encode_one_img(original_img(:,:,1), complementary_principle(1, :));
    green_encoded_img = encode_one_img(original_img(:,:,2), complementary_principle(1, :));
    blue_encoded_img = encode_one_img(original_img(:,:,3), complementary_principle(1, :));
    
    encoded_img(:,:,1) = red_encoded_img;
    encoded_img(:,:,2) = green_encoded_img;
    encoded_img(:,:,3) = blue_encoded_img;
end